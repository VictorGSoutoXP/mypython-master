...
(function($) {
	...
	$(document).ready(function() {
		...
		$(document).on('change', "#laudo_modelo", function(e){
			...
			if(value && value != "-1"){
//				var modelo_content = CKEDITOR.instances.modelo_content.getData();
//				
//				if(modelo_content.length){
//					if(!confirm("Deseja perder o modelo que está em uso.")){
//						return false;
//					}
//				}
...
//					    });
		...
		$(document).on('click', "#laudo_impossibilitado", function(){
			...
			var btnHolter = $("#arquivo_laudo_holter");
			//console.log(btnHolter);
			
			if (exame_id == 9){
				if (checked) $("#laudo_imposs_container").show();
				else $("#laudo_imposs_container").hide();
				return;
			}

			if(checked){
				
				if (btnHolter) btnHolter.attr('required', false); //rules('add', {required: false});
				
				$("#laudo_info_container").stop(true, true).slideUp();
				$("#laudo_imposs_container").stop(true, true).slideDown();
			}else{
				
				if (btnHolter) btnHolter.attr('required', true); //rules('add', {required: true});

				$("#laudo_info_container").stop(true, true).slideDown();
				$("#laudo_imposs_container").stop(true, true).slideUp();
			}
		});
