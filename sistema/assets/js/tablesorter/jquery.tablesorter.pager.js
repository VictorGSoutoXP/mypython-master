	...
	$.extend({ tablesorterPager: new function() {
		...
		updatePageDisplay = function(table, c) {
			...
			if ( Math.min( c.totalPages, c.filteredPages ) > 0 ) {
				...
				if($(c.cssNumPageContainer, c.container).length ){
					var ctp = $(c.cssNumPageContainer, c.container);
					ctp.empty();
					var pv = $('<li><a href="#">«</a></li>').addClass(c.cssPrev.replace(/^\./, ""));
					ctp.append(pv)
					
					p = Math.min( c.totalPages, c.filteredPages );
					for ( i = 1; i <= p; i++ ) {
						var pv = $('<li><a href="#">'+i+'</a></li>').addClass(c.cssNumPage.replace(/^\./, ""));
						if(i == c.page + 1){
							pv.addClass('active');
						}
						ctp.append(pv)
					...
					var nx = $('<li><a href="#">»</a></li>').addClass(c.cssNext.replace(/^\./, ""));
					ctp.append(nx)
				}
			}
			pagerArrows(c);
		...
		hideRowsSetup = function(table, c){
			c.size = parseInt( $(c.cssPageSize, c.wrap).find('option[selected]').val(), 10 ) || c.size;
			$.data(table, 'pagerLastSize', c.size);
			pagerArrows(c);
			if ( !c.removeRows ) {
				hideRows(table, c);
		...
		showAllRows = function(table, c){
			...
			$(c.wrap).find(c.cssPageSize + ',' + c.cssGoto).each(function(){
		...
		enablePager = function(table, c, triggered){
			var p = $(c.cssPageSize, c.wrap).removeClass(c.cssDisabled).removeAttr('disabled');
			$(c.wrap).find(c.cssGoto).removeClass(c.cssDisabled).removeAttr('disabled');
			c.isDisabled = false;
		...
		$this.construct = function(settings) {
			return this.each(function() {
				...
				// goto selector
				if ( c.wrap.find(c.cssGoto).length ) {
					c.wrap.find(c.cssGoto)
						.unbind('change')
				...
				// list seletor
				if ( pager.find(c.cssNumPage).length ) {
					pager.find(c.cssNumPage)
