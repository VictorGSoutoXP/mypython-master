...
class G_laudos {
	...
	function gerar_laudo_via_pdf($id){
		...
		if ($this->assinarLaudoDigital($exame['arquivo_laudo'], $cert))
			$this->CI->pacientes_exames_model->update(array('protocolo' => $cert['protocolo']), $id);
	}


	
	/*-------------------VINICIUS------------------------*/
	function gerar_laudo_ecg($id){
		$arquivo_laudo = null;
        ...
        $exame['cliente'] = $cliente;
 		...
 		/*Aqui é onde ACHAMOS que pode vir em análise os dados preenchidos nestes campos, mas não deu certo*/

 		/*$exame['laudo_ecg'] = $laudo_ecg;
        $exame['frequencia_ecg'] = $frequencia_ecg;
        $exame['laudo_ecg_outros'] = $laudo_ecg_outros;*/
	...
	function gerar_laudo($id_exame){
		...
		//log_message('debug', 'CLIENTE');
		//log_message('debug', print_r($cliente, true));
		//log_message('debug', '===================================');
