...
class G_laudos {
	...
	function gerar_laudo($id_exame){
		...
		if ($exame){
			...
			log_message('debug', $content); 
			try{ 
				$mpdf->WriteHTML("HOSTGATOR MALDITA!"); // $content);
			} catch (Exception $e){
				echo "ERRO: " . $e->getMessage();
				log_message("ERRO ERRO ERRO ERRO ERRO ERRO ERRO ERRO ERRO ERRO ERRO ERRO ERRO ");
			...
			if ($this->dest_path){
				...
				log_message('debug', '########################################################################### 4 - ' . $this->dest_path . $name);
