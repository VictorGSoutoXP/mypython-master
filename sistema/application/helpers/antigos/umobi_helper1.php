...
{
	$ci =& get_instance();
	$c_exame = $ci->clientes_exames_model->get(array('cliente_id' => $exame['cliente_id'], 'exame_id' => $exame['exame_id']));
	$preco = 0;
	$preco_medico = 0;
...
function download_exame($exame){
    $url = config_item('upload_folder')."exames/" . $exame['arquivo_exame'];
...
function get_count_waiting() {
	...
	$exames = $ci->pacientes_exames_model->get_all ( array("status" => "0", "ativo" => 1) );
