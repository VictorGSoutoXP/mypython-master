...
function calculate_years_old($birthday, $currentday = null){
...
function calculate_years_old_OLD($birthday, $current_day){
	if (!$birthday || $birthday == "0000-00-00") return "";
...
function download_exame($exame){
    ...
    $isFileName = strpos($exame['arquivo_imagem'], '{{') === false;
    if (!IsNullOrEmptyString($exame['arquivo_imagem']) && $isFileName) $arquivo = $exame['arquivo_imagem'];
    //log_message('debug', 'AQUIhelpersumobi_helper>>>>>>' . $arquivo);
...
function get_count_waiting() {
	...
	$exames = $ci->pacientes_exames_model->get_all( array("status" => "0", "ativo" => 1, "expirando" => true, 'todos' => true) );
...
function renderLaudoECGView($exame){
	
	$dados_ecg = $exame['dados_ecg'];
	$selecionados = $exame['laudo_ecg'] ? ';' . $exame['laudo_ecg'] .';' : null;
	$content = "";
	
	for($i=0;$i<count($dados_ecg);$i++){
		$item = $dados_ecg[$i];
		$type = $item['multiplo'] == "S" ? "checkbox" : "radio";
		$id_item = 'laudo_ecg';
		$content .= "<b>" . $item["titulo"] . "</b>" . "<br />";
		for ($j=0;$j<count($item["itens"]);$j++){
			$tag    = '<input #disabled# type="#type#" name="#name#" value="#value#" #checked# /> #titulo#';
			$doenca = $item["itens"][$j];
			
			$id = $type == "checkbox" ? $doenca["id"] : 'T' . $doenca["tipo"];
			$name   = $id_item . '[' . $id . ']';
			$value = $doenca["id"] . '|' . $doenca["descricao"];
			$titulo = $doenca["descricao"];
			
			$tag = $exame['status'] == '1' ? 
				   str_replace('#disabled#',"disabled", $tag) :  
				   str_replace('#disabled#',"", $tag);
			
			$target = ';'. $doenca["id"] . ';';
			
			$tag = strpos($selecionados, $target) === false ? 
				   str_replace('#checked#',"",$tag) : 
				   str_replace('#checked#',"checked",$tag);	
				   
			$tag = str_replace('#type#',$type,$tag);
			$tag = str_replace('#name#',$name, $tag);
			$tag = str_replace('#value#',$value, $tag);
			$tag = str_replace('#titulo#',$titulo, $tag);
	...
	$outros = '<textarea #disabled# name="laudo_ecg_outros" cols="30" rows="5">#value#</textarea>';
	$outros = $exame['status'] == '1' ? 
			  str_replace('#disabled#',"disabled", $outros) :  
			  str_replace('#disabled#',"", $outros);
	$outros = str_replace('#value#', $exame['laudo_ecg_outros'], $outros);
	
	$frequencia = '<b>FREQUÊNCIA(bpm)</b><br /><input #disabled# type="text" name="frequencia_ecg" value="#value#" /><br />';
	$frequencia = $exame['status'] == '1' ? 
			str_replace('#disabled#',"disabled", $frequencia) :  
			str_replace('#disabled#',"", $frequencia);
	$frequencia = str_replace('#value#', $exame['frequencia_ecg'], $frequencia);
	
	return $frequencia . $content . $outros;
}
