    ...
    public function exames() {
        ...
        if($post = $this->input->post()){
            ...
            $this->phpexcel->getProperties()->setCreator(config_item("lab_name"))
                ...
                ->setCategory("Relatorio de Exames");
            ...
            if($exames){
                ...
                foreach ($exames as $exame) {
                    ...
                    if($exame['status'] == 1){
                        ...
                        $activeSheet->getStyle("A{$i}:H{$i}")->applyFromArray($styleImpossibilitado);
                    }else{
                        $activeSheet->getStyle("A{$i}:H{$i}")->applyFromArray($styleAguardando);
                    }
        ...
        if($post = $this->input->post()){
            ...
            if($exames){
                ...
                $activeSheet
                    ...
                    ->setCellValue('F1', 'Nome do Exame')
                    ->setCellValue('G1', 'Paciente')
                    ->setCellValue('H1', 'Valor do Exame R$');
                ...
                foreach ($exames as $exame) {
                    ...
                    $activeSheet
                        ...
                        ->setCellValue('F' . $i, $nomeExame)
                        ->setCellValue('G' . $i, $exame['paciente_nome'])
                        ->setCellValue('H' . $i, doubleval($exame['preco_exame']));
                    ...
                    $activeSheet->getStyle('C' . $i)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_DATE_DDMMYYYY);
                    $activeSheet->getStyle('E' . $i)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_DATE_DDMMYYYY);
                    $activeSheet->getStyle('H' . $i)->getNumberFormat()->setFormatCode("R$ * 0.00"); //'R$ * 0.00'
                ...
                $activeSheet->setSharedStyle($this->headerRelStyle, "A1:H1");
                ...
                $excel_file_name = '"RF_' . $nome_cliente . '_' . $post['inicio'] . '.xls"';
        ...
        if($post = $this->input->post()){
            ...
            if($exames){
                ...
                foreach ($exames as $exame) {
                    ...
                    if($i%2){
                        $activeSheet->setSharedStyle($this->oddRelStyle, "A{$i}:G{$i}");
                    }else{
                        $activeSheet->setSharedStyle($this->evenRelStyle, "A{$i}:G{$i}");
                    }
                    
                    $activeSheet->getStyle("A{$i}:G{$i}")->applyFromArray($styleLaudado);
