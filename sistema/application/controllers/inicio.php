<?php
	...
	class Inicio extends Main_Controller {
		...
		public function lista() {
			...
			$filtrado = $params['mes'] || $params['ano'] || $params ['exame'] || $params ['status'];
			$params ['exame_id_min'] = $filtrado ? null : $config['exame_id_min'];
			
			$mensagem = null;
			
			if (UserSession::isMedico()) $mensagem = $config['mensagem_medico'];
			if (UserSession::isCliente()) $mensagem = $config['mensagem_cliente'];
			$params ['mensagem_cliente'] = $mensagem;
		...
		public function expirando() {
			...
			$this->_dataView ['exames'] = $this->pacientes_exames_model->get_all ( 
		array(
		"status" => "0", 
		"ativo" => 1, 
		"expirando" => true,
		"exame_id_min" => $config['exame_id_min'] 
		) 	
		);
        //$this->_dataView ['anos'] = $this->pacientes_exames_model->get_years ();
        //$this->_dataView ['meses'] = $this->pacientes_exames_model->get_months ();
        ...
        $params ['exame'] = $this->input->get ( 'exame' );
        $params ['nome_paciente'] = $this->input->get ( 'paciente' );
