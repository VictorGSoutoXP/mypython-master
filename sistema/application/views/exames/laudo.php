		...
		<div class="span12">
			...
			<fieldset>
				...
				<?php if ($exame['exame_id'] == 6) { ?>
					<div id="form-acuidade-container">
				...
				<div class="row">
					...
					<div class="span4">
						...
						<div style="font-size: 15px; background-color: yellow; color: red;">
			...
			<fieldset>
				...
				<div id="laudo_imposs_container" class="well"
					style="<?php echo $exame['status'] !== 2 ? "display: none;" : ""?>">
				...
				<?php if ($exame['status'] !== 2) { ?>
					...
					<?php if ($exame['exame_id'] == 1 && $exame['medico_modelo_laudo_ecg'] == '1') { ?>
						<div id="form-ecg-container" style="column-count: 3">							
							<?php echo renderLaudoECGView($exame); ?>
						</div>
					<?php } else if ($exame['exame_id'] != 9) { ?>
					
						<div id="laudo_info_container" class="well well-small">
								...
								<?php } else if ($exame['exame_id'] == 5) { ?>
									<select name="laudo_modelo" id="laudo_modelo" class="select-box-auto-width span4 required" value="-2">
										...
										<?php foreach ($modelos_laudo as  $modelo){?>
											<?php 
												$selected = $exame['laudo_padrao'] == null ? '' :
															($exame['laudo_padrao']['id'] == $modelo['id'] ? 'selected' : '');
											?>
											<option <?php echo $selected; ?> value="<?php echo $modelo['id']?>"><?php echo $modelo['title']?></option>
										<?php }?>
									</select>
								<?php } else { ?>
						...
						<div class="row">
							<div class="span6">
								...
								<input id="anormalidade_parenquima" type="radio"  <?php echo $exame['status'] == 2 ? 'disabled="disabled"' : "" ?>  name="exame_oit[anormalidade_parenquima]" onclick="changeSessao2('N')" value="N"
									<?php echo ( $exame_oit['anormalidade_parenquima'] == 'N' || !$exame_oit['anormalidade_parenquima'] ) ? 'checked="checked"' : ''?>> NÃO
							</div>
						</div>
						<hr>
						...
						<div class="row"> 
							<div class="span6"> 
								...
								<input id="anormalidade_pleural" type="radio"  <?php echo $exame['status'] == 2 ? 'disabled="disabled"' : "" ?>  onclick="changeSessao3('N')" name="exame_oit[anormalidade_pleural]" value="N"
									<?php echo ( $exame_oit['anormalidade_pleural'] == 'N' || !$exame_oit['anormalidade_pleural'] ) ? 'checked="checked"' : ''?>> NÃO 
							</div>
						</div>
						<hr>
...
<script>
	...
	$(document).ready(function(){
		...
		const element = document.querySelector("#btnBaixarExame");
		if (element.classList.contains("link-disabled")) return;
