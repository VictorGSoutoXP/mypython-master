...
<?php 
	//$exame['exame_id'] = 9; 
	//$exame['medico_modelo_laudo_ecg'] = 1;
?>
...
-->
				...
				<div class="row">
					<div class="span3">
						<label for="exame[protocolo]">Protocolo</label>
						<input type="text" disabled="disabled" value="<?php echo $exame['protocolo']?>" 
							id="exame[protocolo]" class="span3" />
					</div>
					<!--Thais-->
					<?php if(UserSession::isAdmin()){?>
					<div class="span4">
						<label for="exame[cliente]">Cliente</label>						
						<input type="text" disabled="disabled" value="<?php echo $exame['cliente']?>" 
							id="exame[cliente]" class="span4" />
					</div>
					<?php }?>
				</div>
				...
				<div class="row">
    	    		...
    	    		<div class="span8">
						...
						<?php 
							...
							$via = $exame['enviado_por'] == "DAMA_DESKTOP" ? "DAMA DESKTOP" :
								   ($exame['enviado_por'] == "LOTE" ? "LOTE" : null);
							
							if ($via !== null) {
								echo '<span style="color:red;font-size:14px"><strong>EXAME RECEPCIONADO VIA ' . $via. '.</strong>';
							}
						?>
		    ...
		    <?php if($exame['status'] <> 0){?>
				...
				<?php if($exame['status'] == 1){?>
					...
					<!-- LAUDO ECG -->
					<?php if($exame['exame_id'] == 1 && strlen(trim($exame['laudo_ecg'])) > 0) { ?> 
							<div id="laudo-ecg-container" style="column-count: 3">
				...
				<fieldset>
                    ...
                    <?php } else {
                        for ($i=0; $i < $arquivos;$i++) { ?>
                            <img src="<?php echo site_url("uploads/dama/exames/" . $nome . '_' . $i . $ext) ?>" alt="Arquivo de imagem"><hr>
                    <?php } } ?>
				</fieldset>
				<?php }  ?>
			</div>
