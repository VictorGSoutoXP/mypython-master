...
<div id="areaA" style="height: 1750px; width: 1250px;"></div>
...
<div id="exame_container" style="display: none;">
		...
		<form action="<?php echo site_url("exames/laudo");?>" method="post" name="enviaLaudo" id="enviaLaudo" enctype="multipart/form-data">
			...
			<div class="span12">
				...
				<div class="form-actions">
					...
					<button id="btnEnviarLaudo" type="submit" class="btn btn-primary pull-right">Enviar Laudo</button>
				</div>
			</div>
		</form>
...
/*
		function doDownloadLaudo(laudo, fn){
			if (!laudo) {
				fn();
				return;
			}

			var url = $("#enviaLaudo").attr("action").replace('/exames/laudo', '/uploads/dama/laudos');
			window.open(url + "/" + laudo); // abre em uma nova aba!
            fn();

		}

		$("#enviaLaudo").submit(function(){

			$.ajax({
				url: $("#enviaLaudo").attr("action"),
				type: 'POST',
				data : $("#enviaLaudo").serialize(),
				success: function(res){
					try {
						...
						doDownloadLaudo(res.laudo, function () {
