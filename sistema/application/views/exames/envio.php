	...
	<form action="<?php echo site_url("exames/envio");?>" method="post" name="enviaExame" id="enviaExame" enctype="multipart/form-data">
	    <div class="span12">
		    <fieldset>
				...
				<input type="hidden" value="<?php echo $usuario['laudos_rapido']?>" id="usuario_laudos_rapido" />
			...
			<div class="form-fields-container" style="display: none;">
			    ...
			    <fieldset>
					...
					<div class="row">
						<div class="span3">
							<div id="medico_box"></div>
						</div>
					</div>

					<div class="row">

						<input type="text" id="exame[medico_solicitante]" name="exame[medico_solicitante]" value="1" style="display: none;" />
<!--
						<div class="span3">
...
-->
						...
						<div class="span2">
							...
							<input type="text" id="exame[data_exame]" class="span2 data required" name="exame[data_exame]" value="<?php echo date('d/m/Y')?>" /><br />
						</div>
					...
					<div>
						<span style="color: red">
							<strong>Atenção!</strong><br />
							Altere a data do exame acima quando necessário, pois é ela que sairá no laudo!
						</span>
					</div>
			    	...
			    	<div class="row">
			    		<div class="span4">
							...
							<div id = "box_laudo_rapido" >
								<label class="checkbox" for="exame[emergencia]" >
								  <input type = "checkbox" id = "exame[emergencia]" name = "exame[emergencia]" value = "1" > Laudo rápido?
								</label >
							</div >

							<div id = "box_reenviar_exame" >
								<label class="checkbox" for="exame[reenviar_exame]" >
								  <input type = "checkbox" id = "exame[reenviar_exame]" name = "exame[reenviar_exame]" value = "S" > Reenviar Exame?
								</label >
							</div >
			    		...
			    		<div class="span7">
			                ...
			                <div class="fileupload fileupload-new" data-provides="fileupload">
		                        ...
		                        <span id="fileupload-preview" class="fileupload-preview" style="color: red;"></span>
...
<script type="text/javascript">
    ...
    function parseXml(xml) {
		...
		function parseDataNascimento(data) {
			...
			function getAno(ano) {
				...
				return ( parseInt('20' + ano) > hoje ? '19' + ano : '20' + ano);
			}
			...
			const ano = getAno(data.substr(6, 2));
		...
		var empresa = paciente.getElementsByTagName("Endereco")[0].childNodes[0];
        empresa = empresa ? empresa.nodeValue : '-'; //paciente.getElementsByTagName("Endereco")[0].childNodes[0].nodeValue;
        var sexo = paciente.getElementsByTagName("Sexo")[0].childNodes[0].nodeValue;

        var nascimento = parseDataNascimento(
			paciente.getElementsByTagName("DataNascimento")[0].childNodes[0].nodeValue
		);
		
		var motivo = paciente.getElementsByTagName("RegistroClinico")[0].childNodes[0];
		motivo = motivo  ? motivo.nodeValue.replace(/\D/g, '') : null;
		motivo = (motivo && motivo.length > 0) ? motivo : '6';
        ...
        $("#exame\\[motivo\\]").attr('value', motivo);

        $('#paciente\\[nome\\]').attr('value', nome);
        $('#paciente\\[empresa\\]').attr('value', empresa);
    ...
    $(document).ready(function(){
		...
		var medicoForm = [
					{
						view:"text", label:"Nome do Médico", labelPosition: "top", name: "nome", required: true,
						on: {
					...
					{
						view:"text", label:"CRM", labelPosition: "top", width: 100, required: true, name: "crm"
					},
					{ view:"button", label:"Gravar", autowidth: true,
						click: function(){
		...
		var medicoWin = {
			...
			head:{
				view:"toolbar", cols:[
					{ view:"label", label: "Novo Médico" },
					{ view:"button", label: 'Fechar', width: 100, click:"$$('winMedico').hide();", icon:"times", type: "iconButton" }
				]
			},
			body: {
				rows: [
					{ type: "space",
						rows: [
							{
								view: "form", width: 400, height: 200, elements: medicoForm,
								rules: {
									$obj: function(data){

										if (!data.nome || !data.crm) {
											webix.message({type:"error", text:"Todos os campos devem ser preenchidos!"});
											return false;
										}
										ajax.post("/sistema/medicos/incluir", data, function(res) {
											if (!res) return;
											if (res.erro){
												webix.message({type:"error", text: res.erro});
												return;
											}
											webix.message("Novo Médico Incluído!");
											refreshList();
											$('#exame\\[medico_solicitante\\]').attr('value', res.id);
											$$('winMedico').hide();
										});
		...
		var table = {
			...
			//bordless: true,
			columns:[
				{ id:"nome",	header:["Nome", {content:"textFilter"}], width:300,	sort:"string"},
				{ id:"funcao",	header:["Função", {content:"textFilter"}], width:150,	sort:"string"},
										...
										{view:"button", label: "Selecionar Paciente", type:"iconButton", icon: "check-circle",  autowidth: true,
                                            on: {
                                                'onItemClick': function(){
                                                    ...
                                                    $("#paciente\\[funcao\\]").attr('value', p.funcao);
		...
		function refreshList(){
			var list = $$('cbxMedico').getPopup().getList();
			list.clearAll();
			list.load('/sistema/exames/medicos');
		}

		webix.ui({
			container: "medico_box",
			rows:[
				{
					view:"combo", id:"cbxMedico", width:300, label: 'Médico Solicitante',  labelPosition: "top", name:"cbxMedico",
					value:1,
					options:{
						filter:function(item, value){
							if(item.nome.toString().toLowerCase().indexOf(value.toLowerCase())===0)
								return true;
							return false;
						},
						body:{
					...
					on: {
						...
						'onChange':function(cur, old){
							$('#exame\\[medico_solicitante\\]').attr('value', cur);
						}
					}
				},
				{
